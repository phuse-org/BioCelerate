...
# overview of confidence plot algorith 

# each query block 
# define three lists, confidently matching
# confidently non matching, and ambiguous
# if a study 
...
all_confidently_matching <- c()
all_confidently_nonmatching <- c()
all_nonconfident <- c()

db <- dbConnect(RSQLite::SQLite(), dbFullName)
all_studies <- data.table(unique(dbGetQuery(db, 'SELECT DISTINCT STUDYID FROM TS')))$STUDYID
dbDisconnect(db)
...
conf_non_matching_start <- non_matching_start[!is.na(parse_iso_8601(non_matching_start$TSVAL))]

all_confidently_matching <- c(all_confidently_matching, conf_matching_start$STUDYID)
all_confidently_nonmatching <- c(all_confidently_nonmatching, conf_non_matching_start$STUDYID)

ambigious <- all_studies[!(all_studies %in% all_confidently_matching) & !(all_studies %in% all_confidently_nonmatching)]
all_nonconfident <- c(all_nonconfident, ambigious)
