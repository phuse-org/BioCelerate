...
groupSEND <- function(dataset,targetDomain,dmFields=c('SEX','ARMCD','SETCD','USUBJID'),
                      ...
                      tsparam=c('TSPARMCD,TSVAL')) {
  
  # Check to ensure that dataset contains all required domains
  requiredDomains <- c('dm','ex','tx','pp','ta','se','ds','ts')
  ...
  #Check that Trial Design is Parallel in sDesign
  SDesignseq <-match("SDESIGN",Dataset$ts$TSPARMCD)
  if (Dataset$ts$TSVAL[SDesignseq] != "PARALLEL") {
    stop('SEND dataset study design is not parallel!')
  }
  ...
  #Linking subject to dose level/set code using TX Domain 
  #Check that SETCD is in proper format and pull out set codes
  ...
  #                                                       (control treatment currently hard coded)
    if ('TCNTRL' %in% dataset$tx$TXPARMCD) { 
      if (suppressWarnings(is.na(as.numeric(TRTDOS[count])==0))){
        TCNTRL[count] <- NA
      } else if (suppressWarnings(as.numeric(TRTDOS[count])==0)) { 
  ...
  # Create Lookup for dose/unit and match to SET
  SetDose <- rep(NA,length(SETCD)) #Making Lookup-Table of Sets and doses
  i <- 1
  for (dose in SETCD) {
    SetDose[i] <- unique(getFieldValue(dataset$tx,'SET','SETCD',dose))
    i <- i+1
  }
  SetDose <- na.omit(SetDose)
  DoseSET <-data.frame(SETCD,Doses=SetDose) 
  groupedData <- merge(groupedData,DoseSET)
  #Remove "Recovery", "TK", "Non-TK", and gender from DoseSET
  groupedData$Doses <-trimws(sub("Recovery", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub(", NON-TK", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub("TK", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub("Female", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub("Male", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub(", ", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub(",", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub("Vehicle", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub("Low", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub("Medium", "",groupedData$Doses))
  groupedData$Doses <-trimws(sub("High", "",groupedData$Doses))
  
  #Merge Treatment from EXDomain into groupedData
  Treatment <- rep(NA, length(subjects))
  i <-1
  for (subject in subjects){
    Treatment[i] <-unique(getFieldValue(dataset$ex,'EXTRT','USUBJID',subject))
    i <- i+1
  }
  exData <- data.frame(USUBJID=subjects,EXTRT=Treatment)
  ...
  # Merge disposition description and Clean up the dataframe
  ...
  if (is.factor(groupedData$Doses)) {
    if (! FALSE %in% is.finite(as.numeric(levels(groupedData$Doses)[groupedData$Doses]))) {
      groupedData$EXDOSE <- as.numeric(levels(groupedData$Doses)[groupedData$Doses])
    } else {
      groupedData$EXDOSE <- as.character(levels(groupedData$Doses)[groupedData$Doses])
    }
  }
  if (is.factor(groupedData$Doses)) {
    if (! FALSE %in% is.finite(as.numeric(levels(groupedData$Doses)[groupedData$Doses]))) {
      groupedData$TRTDOS <- as.numeric(levels(groupedData$Doses)[groupedData$Doses])
    } else {
      groupedData$TRTDOS <- as.character(levels(groupedData$Doses)[groupedData$Doses])
    }
  }
  if (! FALSE %in% (groupedData$Doses==groupedData$Doses)) {
    dropColumns <- c(dropColumns,'TRTDOS')
  }
  groupedData$Dose <- groupedData$Doses
  
  # Create concatenated dose with units
  groupedData$TreatmentDose <- paste(groupedData$Treatment,groupedData$Doses)
  dropColumns <- c(dropColumns,'Doses')
